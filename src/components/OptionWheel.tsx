...
type DragState = { y: number; start: number; id: number }
...
const OptionWheel = ({
  ...
  const playTick = useCallback(() => {
    const { soundUrl, soundVolume } = cfgRef.current as { soundUrl: string; soundVolume: number }
    if (!soundUrl) return
  ...
  const applyTarget = useCallback(
    (value: number, snap: boolean) => {
      const cfg = cfgRef.current as { loop: boolean; count: number; items: string[] }
      let v = value
  ...
  const handlePointerMove = useCallback(
    (e: React.PointerEvent<HTMLDivElement>) => {
      ...
      if (dragMovedRef.current) {
        const rowH = (cfgRef.current as { rowH: number }).rowH
